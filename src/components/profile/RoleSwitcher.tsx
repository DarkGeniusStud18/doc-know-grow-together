 ...
 * Il nécessite un code PIN et un mot de passe pour autoriser le changement de rôle.
 */
...
const RoleSwitcher: React.FC<RoleSwitcherProps> = ({ inSettings = false }) => {
  // Hooks pour l'état de l'authentification et de l'interface utilisateur
  ...
  /**
   * Effet pour récupérer les identifiants de changement de rôle au montage du composant
   */
  useEffect(() => {
    const fetchCredentials = async () => {
      try {
        ...
        // Requête pour récupérer les derniers identifiants configurés
  ...
  /**
   * Gère le processus de changement de rôle de l'utilisateur
   * Vérifie les identifiants et met à jour le profil
   */
  const handleSwitchRole = async () => {
    ...
    try {
      ...
      // Détermination du nouveau rôle (basculement)
      ...
      // Mise à jour de l'état local de l'utilisateur dans le contexte
      updateCurrentUser({
        ...user,
        role: newRole
      });
      
      // Affichage du message de succès et fermeture du dialog
      const roleText = newRole === 'student' ? 'Étudiant' : 'Professionnel';
      toast.success(`Interface changée avec succès: ${roleText}`);
      setIsDialogOpen(false);
      
      // Réinitialisation des champs de saisie pour la sécurité
      setPinCode('');
      setPassword('');
  ...
  return (
    <>
      {/* Affichage différent selon si c'est dans les paramètres ou non */}
      {inSettings ? (
        <Card>
          <CardContent className="p-6">
            <div className="space-y-4">
              {/* En-tête avec titre et badge de rôle actuel */}
              ...
              {/* Contrôles pour le changement d'interface */}
              <div className="flex items-center justify-between">
                <Switch
                  checked={user.role === 'professional'}
                  disabled // Switch désactivé, utilisé uniquement pour l'affichage
                />
        ...
        // Affichage simple avec bouton uniquement
        <Button
          variant="outline"
          className="flex items-center gap-2"
          onClick={() => setIsDialogOpen(true)}
        >
          {/* Icône de changement d'interface */}
      ...
      {/* Dialog modal pour la saisie des identifiants */}
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-4 py-4">
            ...
            {/* Informations de contact pour obtenir les identifiants */}
