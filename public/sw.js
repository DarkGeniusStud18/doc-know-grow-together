    ...
    Promise.allSettled([
      ...
      caches.open(STATIC_CACHE).then(async (cache) => {
        ...
        const results = await Promise.allSettled(
          ESSENTIAL_URLS.map(url => cache.add(url).catch(err => {
            console.warn(`Service Worker: Impossible de mettre en cache ${url}:`, err);
          }))
        );
        console.log('Service Worker: Ressources essentielles mises en cache:', results.length);
        return results;
      }),
      ...
      caches.open(IMAGE_CACHE).then(async (cache) => {
        ...
        const results = await Promise.allSettled(
          IMPORTANT_IMAGES.map(url => cache.add(url).catch(err => {
            console.warn(`Service Worker: Impossible de mettre en cache l'image ${url}:`, err);
          }))
        );
        console.log('Service Worker: Images importantes mises en cache:', results.length);
        return results;
      })
    ]).then(() => {
...
self.addEventListener('activate', (event) => {
  ...
  const validCaches = [CACHE_NAME, STATIC_CACHE, DYNAMIC_CACHE, IMAGE_CACHE, API_CACHE];
...
async function networkFirstWithOfflineFallback(request, cacheName) {
  try {
    ...
    console.log('Service Worker: Réseau indisponible, vérification du cache...');
...
async function networkFirstStrategy(request, cacheName) {
  try {
    ...
    return cached || new Response('Contenu non disponible hors ligne', { 
      status: 503,
      statusText: 'Service Unavailable',
      headers: { 'Content-Type': 'text/plain; charset=utf-8' }
    });
...
async function networkFirstWithTimeout(request, cacheName, timeout = 5000) {
  try {
    ...
    console.log('Service Worker: Timeout ou erreur réseau, vérification du cache...');
  ...
  if (event.action === 'explore') {
    ...
    event.waitUntil(
      clients.matchAll({ type: 'window' }).then((windowClients) => {
        ...
        console.error('Service Worker: Erreur lors de l\'ouverture de la notification:', error);
      })
    );
  }
});
...
console.log('Service Worker MedCollab v1.3.0 chargé et configuré avec optimisations avancées');
